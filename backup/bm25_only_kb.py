from llama_index import SimpleDirectoryReader, StorageContext, load_index_from_storage, Document
...
class HierarchicalKnowledgeBase:
    ...
    async def create_index(self, data_dir: str, progress_callback: Optional[Callable] = None, skip_if_exists: bool = True):
        ...
        try:
            ...
            with atomic_index_update(self):
                # Create index
                # Store nodes directly since we don't need vector embeddings
                self.docstore = {node.id_: node for node in all_nodes}
                self.index = None  # No vector index needed
                # Persist the index
    ...
    async def retrieve_relevant_nodes(self, query_text: str, similarity_top_k: int = 15):
        ...
        # Use cached BM25 retriever or create new one
        if self._bm25_retriever is None:
            # Get all nodes from the document store
        ...
        nodes = await self._bm25_retriever.aretrieve(query_text)
    ...
    async def get_relevant_context(self, query_text: str, 
        ...
        query_stats['retriever_creation'] = bool(self._bm25_retriever is None)
