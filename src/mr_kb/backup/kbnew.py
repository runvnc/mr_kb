...
class HierarchicalKnowledgeBase:
    ...
    def _migrate_from_old_index(self):
        """Migrate data from old index format to new ChromaDB format"""
        old_index_path = os.path.join(self.persist_dir, "index_store.json")
        if os.path.exists(old_index_path):
            logger.info(f"Migrating from old index format in {self.persist_dir}")
            try:
                # Load old index
                old_storage_context = StorageContext.from_defaults(persist_dir=self.persist_dir)
                ...
                logger.error(f"Failed to migrate from old index: {str(e)}")
                return []
    ...
    async def create_index(self, data_dir: str, progress_callback: Optional[Callable] = None, skip_if_exists: bool = True):
        ...
        try:
            ...
            # Check if we need to migrate from old index
            old_documents = self._migrate_from_old_index()
            ...
            raise DocumentProcessingError(f"Index creation failed: {str(e)}. {traceback.format_exc()}") from e
    ...
    async def add_document(self, file_path: str, 
        ...
        try:            
            ...
            raise DocumentProcessingError(f"Document addition failed: {str(e)}. {traceback.format_exc()}") from e
    ...
    async def remove_document(self, file_path: str, remove_from_metadata_index: bool = True):
    ...
    @dispatcher.span
    async def retrieve_relevant_nodes(self, query_text: str, similarity_top_k: int = 15, final_top_k: int = 6, min_score: float = 0.69, include_verbatim: bool = True, search_metadata: bool = True):
    ...
    async def get_relevant_context(self, query_text: str, 
                            ...
                            include_verbatim: bool = False, search_metadata: bool = True) -> str:
